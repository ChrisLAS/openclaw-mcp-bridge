...
const plugin = {
  ...
  register(api: PluginApi) {
    ...
    for (const server of config.servers) {
      // Log only the server name — URL is sanitized to strip any query params
      api.logger.info(
      ...
      for (const mcpTool of mcpTools) {
        ...
        // Register tool directly — no factory needed
        const tool = createBridgedTool(server, mcpTool, sessionId);
